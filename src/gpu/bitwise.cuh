...
__global__ void bitexpand(T *a, size_t nVals, U *b) {
    ...
    int nBits = sizeof(T) * 8; // get the expected bits: sizeof(uint64_t) = 8 * 8 = 64 bits
...
// invoke order: gpu::bitexpand -> kernel::bitexpand
template<typename T, typename I, typename U, typename I2>
void bitexpand(DeviceData<T, I> *a, DeviceData<U, I2> *b) {

    int cols = a->size();
    int rows = sizeof(T) * 8; //sizeof()=bytes, 1 bytes = 8 bit

    dim3 threadsPerBlock(cols, rows); // Block includes (cols * rows) threads
    dim3 blocksPerGrid(1, 1); // Grid includes (1 * 1 = 1) block

    // Here, MAX_THREADS_PER_BLOCK is ambiguous
    // If threadsPerBlock.x = MAX_THREADS_PER_BLOCK and threadsPerBlock.y =2, then exceed MAX_THREADS_PER_BLOCK
    // Therefore, MAX_THREADS_PER_BLOCK here is self-defined, not the maximum threads per block allowed by the hardware
    ...
    // &a->begin()[0] return the first value in a. &a->begin() is a iterator.
    ...
    cudaThreadSynchronize(); // ensure all the threads finish the operations
}
