...
class DeviceDataBase {
    ...
    public:
        ...
        template<typename I2>
        DeviceDataBase<T, Iterator> &operator|=(const DeviceDataBase<T, I2> &rhs) {
            thrust::transform(this->begin(), this->end(), rhs.begin(), this->begin(), thrust::bit_or<T>());
            return *this;
        }
...
class DeviceData<T, BufferIterator<T> > : public DeviceDataBase<T, BufferIterator<T> > {
    ...
    private:
        // DeviceData class encapsulates thrust::device_vector to remain the data on device
