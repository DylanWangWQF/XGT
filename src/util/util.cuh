...
#define ASSERT_EPSILON 1e-3 // accepted error
...
// Dylan test start
// Integer Test, vector<uint64_t> &host_data
template<typename T, typename I>
void printDeviceDataForInteger(DeviceData<T, I> &device_data)
{
    typedef typename std::make_signed<T>::type S;
    std::vector<uint64_t> host_data(device_data.size());
...
//     for (int i = 0; i < host_data.size(); i++) {
//         // printf("%d ", host_temp[i]);
//         // std::cout << host_temp[i] << " ";
//         // printf("%lld ", host_temp[i]);
//         // printf("%f ", host_temp[i]);
//         printf("%lld ", host_data[i]);
//     }
//     std::cout << std::endl;
