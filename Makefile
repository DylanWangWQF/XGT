...
VPATH             := src/:src/dt:src/gpu:src/mpc:src/util:src/test
...
$(BUILD_DIR)/%.o: %.cu $(HEADER_FILES)
	$(CXX) -dc --extended-lambda $(FLAGS) -Xcompiler="$(PIRANHA_FLAGS)" -c $< -o $@ $(OBJ_INCLUDES)
...
$(DEBUG_DIR)/%.o: %.cu $(HEADER_FILES)
	$(CXX) -dc --extended-lambda $(DEBUG_FLAGS) -Xcompiler="$(PIRANHA_FLAGS)" -c $< -o $@ $(OBJ_INCLUDES)
